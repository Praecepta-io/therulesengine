...
public class PraeceptaSubtractionFactorAction extends PraeceptaNumberFactorAction{
	private static final Logger LOG = LoggerFactory.getLogger(PraeceptaSubtractionFactorAction.class); 
	...
	public PraeceptaActionHolder performAction(Object value, Map<PraeceptaActionParametersType,Object> parameters) {
		...
		if(validateDataType(value, factorValue)) {
			...
			Object actionedValue;
			try {
				actionedValue = new BigDecimal(value.toString()).subtract(new BigDecimal(factorValue.toString()));
				...
				PraeceptaUnderlyingFactorAction.addStatusAndMessageToActonHolder(actionHolder,
						ACTION_RESULT_STATUS.FAILURE,
						"Unable to Convert to Substract Value : " + value + " and Substract Value "+factorValue);
			}
			...
			PraeceptaUnderlyingFactorAction.addStatusAndMessageToActonHolder(actionHolder,
					ACTION_RESULT_STATUS.UNABALE_TO_PERFORM_ACTION,
