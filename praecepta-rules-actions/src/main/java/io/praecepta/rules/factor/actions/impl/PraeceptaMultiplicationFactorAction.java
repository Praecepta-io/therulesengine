...
public class PraeceptaMultiplicationFactorAction extends PraeceptaNumberFactorAction {

	private static final Logger LOG = LoggerFactory.getLogger(PraeceptaMultiplicationFactorAction.class);
	...
	public PraeceptaActionHolder performAction(Object value, Map<PraeceptaActionParametersType,Object> parameters) {
		...
		if (validateDataType(value, factorValue)) {
			...
			try {
				actionedValue = new BigDecimal(value.toString()).multiply(new BigDecimal(factorValue.toString()));
				...
				PraeceptaUnderlyingFactorAction.addStatusAndMessageToActonHolder(actionHolder,
						...
						"Unable to Convert to Multiply Value : " + value + " and Substract Value "+factorValue);
			}
		} else {
