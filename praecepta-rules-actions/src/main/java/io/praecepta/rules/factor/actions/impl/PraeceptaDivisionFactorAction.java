...
public class PraeceptaDivisionFactorAction extends PraeceptaNumberFactorAction{
	
	private static final Logger LOG = LoggerFactory.getLogger(PraeceptaDivisionFactorAction.class); 
	...
	public PraeceptaActionHolder performAction(Object value, Map<PraeceptaActionParametersType,Object> parameters) {
		...
		if(validateDataType(value, factorValue)) {
			...
			try {
				actionedValue = new BigDecimal(value.toString()).divide(new BigDecimal(factorValue.toString()));
				...
				PraeceptaUnderlyingFactorAction.addStatusAndMessageToActonHolder(actionHolder,
						...
						"Unable to Convert to Divide Value : " + value + " and Substract Value "+factorValue);
			}
		}else {
