...
public class PraeceptaMinutesSubtractionFactorAction  extends PraeceptaDateFactorAction{
	private static final Logger LOG = LoggerFactory.getLogger(PraeceptaMinutesSubtractionFactorAction.class); 
	...
	public PraeceptaActionHolder performAction(Object value, Map<PraeceptaActionParametersType,Object> parameters) {
		...
		if (validateDataType(value, factorValue)) {
			...
			if (date != null) {
				...
				c.setTime(date);
				c.add(Calendar.MINUTE, Integer.valueOf(factorValue.toString()) * -1);
