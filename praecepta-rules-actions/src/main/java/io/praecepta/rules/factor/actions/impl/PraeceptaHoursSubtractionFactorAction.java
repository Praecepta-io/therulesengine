...
public class PraeceptaHoursSubtractionFactorAction  extends PraeceptaDateFactorAction{
	private static final Logger LOG = LoggerFactory.getLogger(PraeceptaHoursSubtractionFactorAction.class); 
	...
	public PraeceptaActionHolder performAction(Object value, Map<PraeceptaActionParametersType,Object> parameters) {
		...
		if (validateDataType(value, factorValue)) {
			...
			if (date != null) {
				...
				c.setTime(date);
				c.add(Calendar.HOUR, Integer.valueOf(factorValue.toString()) * -1);
