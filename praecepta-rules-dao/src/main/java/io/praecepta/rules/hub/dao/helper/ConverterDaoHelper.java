...
public class ConverterDaoHelper {
	...
	public static List<PraeceptaRuleSpace> getRuleSpaceList(Collection<PraeceptaRuleSpaceDbModel> ruleSpaceModels) {
		...
		if (!ObjectHelper.isObjectEmpty(ruleSpaceModels)) {
	...
	public static String prepareKeyRuleSpaceIdMap(PraeceptaRuleSpaceCompositeKey key, String version) {
		...
		} else if (!ObjectHelper.isObjectEmpty(key.getVersion())) {
	...
	public static PraeceptaRuleSpaceDbModel prepareRuleSpaceModel(PraeceptaRuleSpace ruleSpace,Map<String, Long> ruleSpaceIdByCompositeKeyMap) {
		...
		if (!ObjectHelper.isObjectNull(ruleSpaceId)) {
	...
	public static void prepareListRuleSpaceIdsByKey(PraeceptaRuleSpaceCompositeKey key,
		...
		Long ruleSpaceId = ruleSpaceIdByCompositeKeyMap.get(prepareKeyRuleSpaceIdMap(key));
		if (!ObjectHelper.isObjectNull(ruleSpaceIdsToDelete)) {
	...
	public static String prepareKeyRuleGroupIdMap(PraeceptaRuleSpaceCompositeKey key,
		...
		if (!ObjectHelper.isObjectEmpty(key.getVersion())) {
			keyBuilder.append("_").append(key.getVersion());
		}
		if (!ObjectHelper.isObjectEmpty(ruleGroupName)) {
	...
	public static PraeceptaRuleGroup convertModelToRuleGroup(PraeceptaRuleGroupDbModel objRuleGroupModel) {
		...
		PraeceptaRuleGroup ruleGroupInfo=gsonHelperWithAdapter.toEntity(objRuleGroupModel.getRules(), PraeceptaRuleGroup.class);
		
		if(!ObjectHelper.isObjectNull(ruleGroupInfo)) {
    ...
    public static PraeceptaRuleGroupDbModel populateRuleGroupDbModel(PraeceptaRuleGroup ruleGroup) {
		...
		if (!ObjectHelper.isObjectNull(ruleGroup.getRuleSpaceKey())
				&& !ObjectHelper.isObjectNull(ruleGroup.getRuleSpaceKey().getVersion())) {
    ...
    public static PraeceptaRuleGroupDbModel convertToRuleGroupDbModel(PraeceptaRuleGroup ruleGroup,Map<String, Long> ruleSpaceIdByCompositeKeyMap,Map<String, Long> ruleGrpIdByKeyAndGrpNameMap) {
		...
		if (!ObjectHelper.isObjectNull(ruleGroupId)) {
			rgModel.setId(ruleGroupId);
		}else if (!ObjectHelper.isObjectEmpty(ruleGroup.getUniqueId())) {
	...
	public static Predicate<PraeceptaRuleSpaceDbModel> matchRuleSpaceKey(PraeceptaRuleSpaceCompositeKey key){
		...
		return (obj)->{
			if (obj.getSpaceName().equalsIgnoreCase(key.getSpaceName())
				...
				if (!ObjectHelper.isStringEmpty(key.getVersion())) {
	...
	private static boolean ruleGroupNameMatch(String ruleGroupName,PraeceptaRuleGroupDbModel obj) {
		if (!ObjectHelper.isStringEmpty(ruleGroupName)) {
	...
	public static PraeceptaRuleSideCars convertModelToRuleSideCars(PraeceptaRuleSideCarsDbModel objRuleSideCarModel) {

		PraeceptaRuleSideCars ruleSideCar = new PraeceptaRuleSideCars(objRuleSideCarModel.getSpaceName(),
		...
		ruleSideCar.setRuleGroupSideCars(gsonHelperWithAdapter.toEntity(objRuleSideCarModel.getMetaData(), PraeceptaRuleGroupSideCars.class));
	...
	public static PraeceptaRuleSideCarsDbModel populateRuleSideCarDbModel(PraeceptaRuleSideCars ruleSideCar) {
		...
		if (!ObjectHelper.isObjectNull(ruleSideCar.getRuleSpaceKey())
				&& !ObjectHelper.isObjectNull(ruleSideCar.getRuleSpaceKey().getVersion())) {
			ruleSideCarDbModel.setVersion(ruleSideCar.getRuleSpaceKey().getVersion());
		}
		return ruleSideCarDbModel;
    }
	
	public static PraeceptaRuleSideCarsDbModel convertToRuleSideCarDbModel(PraeceptaRuleSideCars ruleSideCar,Map<String, Long> ruleSideCarIdByKeyAndGrpNameMap) {
		...
		if (!ObjectHelper.isObjectNull(ruleSideCarId)) {
