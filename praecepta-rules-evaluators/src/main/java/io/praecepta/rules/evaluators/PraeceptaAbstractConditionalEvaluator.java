...
//	private boolean inputValidationDone;
	...
	public final boolean validateInput(INPUT input) {
		...
		/*if(ConditionOperatorType.EQUAL_CHARS == input.getConditionOperator() && !ObjectHelper.isObjectEmpty(input.getSubjectName()) &&
				(!ObjectHelper.isObjectEmpty(input.getValueToCompare()) || !ObjectHelper.isObjectEmpty(input.getDefinedAttributeToCompare()))
				) {
			isAValidInput = true;
		}*/
		
		if(!ObjectHelper.isObjectEmpty(input.getConditionOperator()) && validateConditionOperatorType(input.getConditionOperator()) 
				&& !ObjectHelper.isObjectEmpty(input.getConditionValueHolder()) && validateDataType(input.getConditionValueHolder())
		){
			isAValidInput = true;
		}
