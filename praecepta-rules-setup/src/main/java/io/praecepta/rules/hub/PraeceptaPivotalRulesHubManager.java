...
public class PraeceptaPivotalRulesHubManager implements IPraeceptaPivotalRulesHubManager{
	...
	public boolean createRuleGrp(PraeceptaRuleSpaceCompositeKey spaceKey, String version, String ruleGrpName,
		...
		if(isTheRuleSpaceExist(spaceKey, version)) {
			...
			if(ruleGrp != null && !ObjectHelper.isObjectEmpty(ruleGrp.getPraeceptaCriterias())) {
	...
	@Override
	public boolean createRuleSideCars(PraeceptaRuleSideCars ruleSideCar) {
	...
	public boolean updateRuleSideCars(PraeceptaRuleSpaceCompositeKey spaceKey, String version, String ruleGrpName,
		...
		PraeceptaRuleSideCars ruleSideCarFrmDB = fetchRuleSideCars(spaceKey, version, ruleGrpName);
	...
	@Override
	public PraeceptaRuleSideCars fetchRuleSideCars(PraeceptaRuleSpaceCompositeKey spaceKey, String version,
			String ruleGrpName) {
	...
	@Override
	public List<PraeceptaRuleSideCars> fetchRuleSideCars(PraeceptaRuleSpaceCompositeKey spaceKey, String version) {
		if(spaceKey != null && !ObjectHelper.isObjectEmpty(version)) {
	...
	public boolean deleteSideCars(PraeceptaRuleSpaceCompositeKey spaceKey, String version, String ruleGrpName) {
		PraeceptaRuleSideCars ruleSideCarFromDB = fetchRuleSideCars(spaceKey, version, ruleGrpName);
	...
	public boolean deleteRuleSideCars(PraeceptaRuleSpaceCompositeKey spaceKey, String version) {
		List<PraeceptaRuleSideCars> ruleSideCarsFrmDB = fetchRuleSideCars(spaceKey, version);
	...
	@Override
	public boolean deleteRuleSideCars(PraeceptaRuleSideCars ruleSideCar) {
	...
	public void fetchActiveUniverse() {
		...
		Map<PraeceptaRuleSpaceCompositeKey, Map<String, Collection<PraeceptaRuleSideCars>>> ruleSideCarMap = new HashMap<>();
		...
		Collection<PraeceptaRuleSideCars> activeSidecCars = ruleSideCarsDao.fetchAll();
		...
		if(!ObjectHelper.isObjectEmpty(activeSidecCars)) {
			...
			activeSidecCars.stream().forEach( eachSideCar -> {
				...
				ruleSideCarMap.putIfAbsent(spaceKey, new HashMap<String, Collection<PraeceptaRuleSideCars>>());
				
				Map<String, Collection<PraeceptaRuleSideCars>> versionToSideCarMap = ruleSideCarMap.get(spaceKey);
				
				versionToSideCarMap.putIfAbsent(spaceKey.getVersion(), new ArrayList<>());
				
				Collection<PraeceptaRuleSideCars> ruleSideCarsForAVersion = versionToSideCarMap.get(spaceKey.getVersion());
