...
public class PraeceptaPivotalRulesHubStore {
	...
	Map<PraeceptaRuleSpaceCompositeKey, Map<String, Collection<PraeceptaRuleSideCars>>> ruleSideCarData = new HashMap<>();
	...
	public PraeceptaPivotalRulesHubStore(
			...
			Map<PraeceptaRuleSpaceCompositeKey, Map<String, Collection<PraeceptaRuleSideCars>>> ruleSideCarData
			) {
	...
	public Map<PraeceptaRuleSpaceCompositeKey, Map<String, Collection<PraeceptaRuleSideCars>>> getRuleSideCarData() {
	...
	public Collection<PraeceptaRuleSpace> getAllActiveRuleSpaces(){
		...
		if(!ObjectHelper.isObjectEmpty(ruleSpacesData)) {
	...
	public Collection<PraeceptaRuleGroup> getAllActiveRuleGrps(){
		...
		if(!ObjectHelper.isObjectEmpty(ruleGrpData)) {
			...
			ruleGrpData.forEach( (spaceKey, versionWithGrp) -> {
				...
				if(!ObjectHelper.isObjectEmpty(ruleGrpsForAVersion)) {
	...
	public Collection<PraeceptaRuleSideCars> getAllActiveRuleSideCars(){
		logger.debug("Inside getAllActiveRuleSideCars");
		
		Collection<PraeceptaRuleSideCars> activeRuleSideCars = new ArrayList<>();
		
		if(!ObjectHelper.isObjectEmpty(ruleSideCarData)) {
			
			logger.info("Getting All Active Rule Side Cars ");
			
			ruleSideCarData.forEach( (spaceKey, versionWithSideCar) -> {
				
				Collection<Collection<PraeceptaRuleSideCars>> ruleSideCarsForAVersion = versionWithSideCar.values();
				
				if(!ObjectHelper.isObjectEmpty(ruleSideCarsForAVersion)) {
	...
	public Collection<PraeceptaRuleSpace> getAllActiveRuleSpacesWithRuleGrp(){
		...
		if(!ObjectHelper.isObjectEmpty(ruleSpacesData)) {
			...
			ruleSpacesData.forEach( (spaceKey, versionWithSpace) -> {
				...
				if(!ObjectHelper.isObjectEmpty(ruleGrpsWithDifferentVersionsForAKey)) {
	...
	public PraeceptaRuleSpace getActiveRuleSpacesWithVersion(PraeceptaRuleSpaceCompositeKey compositeKey, String version){
		...
		if(!ObjectHelper.isObjectEmpty(ruleSpacesData) && !ObjectHelper.isObjectEmpty(compositeKey) && !ObjectHelper.isObjectEmpty(version)) {
			...
			if(!ObjectHelper.isObjectEmpty(ruleSpaceWithVersions)) {
				
				ruleSpace = ruleSpaceWithVersions.get(version);
				
				if(!ObjectHelper.isObjectEmpty(ruleSpace)) {
	...
	public PraeceptaRuleGroup getRuleGrpForAndSpaceVersionAndGroupName(PraeceptaRuleSpaceCompositeKey compositeKey, String version, String ruleGroupName){
		...
		if (!ObjectHelper.isObjectNull(ruleSpace)) {
	...
	public Collection<PraeceptaRuleSideCars> getActiveSidecarsForARuleSpaces(PraeceptaRuleSpaceCompositeKey compositeKey, String version){
		logger.debug("Inside getActiveSidecarsForARuleSpaces");
		
		Collection<PraeceptaRuleSideCars> sideCarsForAKeyAndVersion = Collections.emptyList();
		...
		if(ruleSpace != null) {
			...
			Map<String, Collection<PraeceptaRuleSideCars>> sideCarsWithDifferentVersionsForAKey = ruleSideCarData.get(compositeKey);
			
			if(!ObjectHelper.isObjectEmpty(sideCarsWithDifferentVersionsForAKey)) {
				...
				if(!ObjectHelper.isObjectEmpty(sideCarsForAKeyAndVersion)) {
	...
	public PraeceptaRuleSideCars getSideCarsForSpaceAndVersionAndGroupName(PraeceptaRuleSpaceCompositeKey compositeKey, String version, String ruleGroupName){
		logger.debug("Inside getSideCarsForSpaceAndVersionAndGroupName");
		
		Collection<PraeceptaRuleSideCars> sideCarsForAKeyAndVersion = getActiveSidecarsForARuleSpaces(compositeKey, version);
		
		PraeceptaRuleSideCars sideCarsToReturn = null;
		
		if (!ObjectHelper.isObjectNull(sideCarsForAKeyAndVersion)) {
			
			logger.info("  Filtering the Side Cars for Rule grp name - {} ", ruleGroupName);
					
			Optional<PraeceptaRuleSideCars> sideCarsForARuleGrpName = sideCarsForAKeyAndVersion.stream()
					.filter(sideCars -> 
						(
